    ...
    <header class="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-md shadow-sm">
        <div class="max-w-5xl mx-auto px-4 sm:px-5 h-[3.25rem] sm:h-14 flex items-center justify-between">
            <div class="flex items-center gap-5 sm:gap-8 min-w-0">
                <a href="<?php echo e(route('home')); ?>" class="flex items-center gap-2.5 font-semibold text-slate-700 truncate group">
                    ...
                    <span class="truncate group-hover:text-indigo-600 transition-colors"><?php echo e(config('app.name')); ?></span>
                </a>
                <nav class="hidden sm:flex items-center gap-1.5 text-sm">
                    <a href="<?php echo e(route('home')); ?>"
                       class="px-4 py-2 rounded-full font-medium transition-all <?php echo e(request()->routeIs('home') || request()->routeIs('article.*') ? 'bg-blue-50/80 text-blue-700 ring-1 ring-blue-200/60' : 'text-slate-600 hover:text-blue-600 hover:bg-blue-50/50'); ?>">博客</a>
                    <a href="<?php echo e(route('moments')); ?>"
                       class="px-4 py-2 rounded-full font-medium transition-all <?php echo e(request()->routeIs('moments') || request()->routeIs('post.*') ? 'bg-teal-50/80 text-teal-700 ring-1 ring-teal-200/60' : 'text-slate-600 hover:text-teal-600 hover:bg-teal-50/50'); ?>">动态</a>
                    <a href="<?php echo e(route('about')); ?>"
                       class="px-4 py-2 rounded-full font-medium transition-all <?php echo e(request()->routeIs('about') ? 'bg-amber-50/80 text-amber-700 ring-1 ring-amber-200/60' : 'text-slate-600 hover:text-amber-600 hover:bg-amber-50/50'); ?>">关于我</a>
                </nav>
            </div>
            <div class="flex items-center gap-3 shrink-0">
                <nav class="flex sm:hidden items-center gap-1 text-sm">
                    <a href="<?php echo e(route('home')); ?>" class="px-2.5 py-1.5 rounded-lg transition-colors <?php echo e(request()->routeIs('home') || request()->routeIs('article.*') ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-slate-500 hover:text-blue-600'); ?>">文</a>
                    <a href="<?php echo e(route('moments')); ?>" class="px-2.5 py-1.5 rounded-lg transition-colors <?php echo e(request()->routeIs('moments') || request()->routeIs('post.*') ? 'bg-teal-50 text-teal-700 font-semibold' : 'text-slate-500 hover:text-teal-600'); ?>">动</a>
                    <a href="<?php echo e(route('about')); ?>" class="px-2.5 py-1.5 rounded-lg transition-colors <?php echo e(request()->routeIs('about') ? 'bg-amber-50 text-amber-700 font-semibold' : 'text-slate-500 hover:text-amber-600'); ?>">我</a>
                </nav>
                <?php if(auth()->guard()->check()): ?>
                    <?php if(auth()->user()->isAdmin()): ?>
                        <a href="<?php echo e(route('admin.dashboard')); ?>" class="hidden sm:inline text-xs text-stone-400 hover:text-violet-600 transition-colors">后台</a>
                    <?php endif; ?>
                    <a href="<?php echo e(route('profile.edit')); ?>" class="flex items-center ring-2 ring-amber-200/70 rounded-full">
                        <?php if (isset($component)) { $__componentOriginal8ca5b43b8fff8bb34ab2ba4eb4bdd67b = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal8ca5b43b8fff8bb34ab2ba4eb4bdd67b = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.avatar','data' => ['user' => auth()->user(),'size' => 'w-8 h-8','text' => 'text-xs']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['user' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(auth()->user()),'size' => 'w-8 h-8','text' => 'text-xs']); ?>
...
<?php endif; ?>
                ...
                <?php else: ?>
                    <a href="<?php echo e(route('login')); ?>" class="text-sm font-medium bg-gradient-to-r from-indigo-600 to-violet-600 text-white px-3.5 py-1.5 rounded-full hover:from-indigo-500 hover:to-violet-500 shadow-md shadow-indigo-500/20 transition-all">登录</a>
                <?php endif; ?>
            </div>
        </div>
    </header>
        ...
        <div class="border-t border-slate-200/80 bg-white/60 backdrop-blur-md py-10">
            <div class="max-w-5xl mx-auto px-4 sm:px-5">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                    <div>
                        <p class="font-title text-lg text-indigo-900/80"><?php echo e(config('app.name')); ?></p>
                        <p class="text-sm text-slate-500 mt-1">记录文字与日常</p>
                    </div>
                    <nav class="flex flex-wrap gap-x-6 gap-y-2 text-sm">
                        <a href="<?php echo e(route('home')); ?>" class="text-slate-500 hover:text-blue-600 transition-colors">文章</a>
                        <a href="<?php echo e(route('moments')); ?>" class="text-slate-500 hover:text-teal-600 transition-colors">动态</a>
                        <a href="<?php echo e(route('about')); ?>" class="text-slate-500 hover:text-amber-600 transition-colors">关于我</a>
                        <?php if(auth()->guard()->guest()): ?>
                            <a href="<?php echo e(route('login')); ?>" class="text-slate-500 hover:text-indigo-600 transition-colors">登录</a>
                        <?php endif; ?>
                        <?php if(auth()->guard()->check()): ?>
                            <a href="<?php echo e(route('profile.edit')); ?>" class="text-slate-500 hover:text-indigo-600 transition-colors">资料</a>
                        <?php endif; ?>
                    </nav>
                </div>
...
</html><?php /**PATH /var/www/ydxred.com/resources/views/front/layout.blade.php ENDPATH**/ ?>