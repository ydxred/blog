...
@section('title', $currentTag ? '动态 · 标签：' . $currentTag : '动态')
@section('description', '碎碎念与日常：捕捉瞬间的灵感、生活的切片以及那些无需长篇大论的思绪。')
...
@section('hero')
    <x-front.hero
        title="碎碎念与日常"
        subtitle="捕捉瞬间的灵感、生活的切片以及那些无需长篇大论的思绪。"
        theme="teal" />
@endsection
...
<div class="max-w-5xl mx-auto">
        ...
        <form action="{{ route('post.store') }}" method="POST" enctype="multipart/form-data" x-ref="form">
            ...
            <div class="flex gap-4 sm:gap-5">
                ...
                <div class="flex-1 min-w-0 pt-1">
                    <label for="post-content" class="sr-only">动态内容</label>
                    <textarea id="post-content" name="content" rows="3" x-model="content" x-ref="textarea"
                    ...
                    <div x-show="previews.length > 0" class="flex flex-wrap gap-2.5 mt-3">
                        <template x-for="(preview, index) in previews" :key="index">
                            <div class="relative w-24 h-24 rounded-xl overflow-hidden ring-1 ring-slate-200 group">
                                <img :src="preview" class="w-full h-full object-cover" alt="即将上传的图片预览">
                    ...
                    <input type="file" name="images[]" multiple x-ref="fileInput" class="hidden" aria-hidden="true" tabindex="-1">
                    ...
                    <div class="flex items-center justify-between mt-5 pt-4 border-t border-slate-100">
                        <div class="flex gap-1.5">
                            <label class="cursor-pointer p-2 rounded-lg text-slate-500 hover:bg-teal-50 hover:text-teal-600 transition-colors" title="添加图片" aria-label="添加图片">
                            ...
                            <button type="button" @click="toggleEmoji()"
                                    ...
                                    :class="showEmoji ? 'text-teal-600 bg-teal-50' : ''"
                                    :aria-pressed="showEmoji ? 'true' : 'false'"
                                    aria-label="添加表情" title="添加表情">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            </button>
                            <button type="button" @click="showTags = !showTags; showEmoji = false"
                                    class="p-2 rounded-lg text-slate-500 hover:bg-teal-50 hover:text-teal-600 transition-colors"
                                    :class="showTags ? 'text-teal-600 bg-teal-50' : ''"
                                    :aria-pressed="showTags ? 'true' : 'false'"
                                    aria-label="添加标签" title="添加标签">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
                            </button>
                        </div>
    ...
    @if($tags->count())
    <nav class="flex flex-wrap gap-2.5 mb-8" aria-label="标签筛选">
        ...
        @foreach($tags as $tag)
            <x-front.tag-pill :tag="$tag" :href="route('moments', ['tag' => $tag->slug])" :active="$currentTag === $tag->slug" size="lg" />
        @endforeach
    </nav>
    @endif
    ...
    <div id="load-more-wrapper" class="mt-10 flex flex-col items-center gap-3">
        @if($posts->hasMorePages())
            <button id="load-more-btn"
                ...
                <svg id="load-more-spinner" class="hidden w-4 h-4 animate-spin text-teal-600" fill="none" viewBox="0 0 24 24" aria-hidden="true">
