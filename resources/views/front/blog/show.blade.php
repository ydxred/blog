...
<div class="flex flex-col lg:flex-row gap-8 max-w-5xl mx-auto items-start">
    ...
    <article class="rounded-2xl border border-slate-200/80 bg-white/90 backdrop-blur-sm p-6 sm:p-10 shadow-sm mb-10 relative overflow-hidden">
            ...
            <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate-500">
                <span class="flex items-center gap-2">
                    @if($article->user->avatar)
                        <img src="{{ asset('storage/' . $article->user->avatar) }}" alt="" class="w-8 h-8 rounded-full object-cover ring-2 ring-indigo-50">
                    @endif
        ...
        @if($article->cover_image)
            <div class="mb-10 rounded-xl overflow-hidden ring-1 ring-slate-200/60 shadow-sm">
                <img src="{{ asset('storage/' . $article->cover_image) }}" alt="" class="w-full object-cover max-h-[32rem]">
            </div>
        @endif
        ...
        <div class="mt-12 pt-6 border-t border-slate-200/80 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-4">
                <button type="button" class="like-btn inline-flex items-center gap-2 px-4 py-2 rounded-full ring-1 ring-slate-200 bg-white/50 text-slate-500 hover:text-pink-500 hover:ring-pink-200 transition-all font-medium text-sm" data-type="article" data-id="{{ $article->id }}">
                ...
                <a href="{{ route('article.show', $related->slug) }}" class="group flex gap-4 rounded-xl border border-stone-200/80 bg-white/60 p-3 hover:bg-white hover:border-indigo-200 hover:shadow-md hover:shadow-indigo-500/5 transition-all">
                    @if($related->cover_image)
                        <div class="w-24 h-20 shrink-0 rounded-lg overflow-hidden bg-stone-100 ring-1 ring-stone-200/50">
                            <img src="{{ asset('storage/' . $related->cover_image) }}" alt="" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                        </div>
                    @endif
    ...
    <section class="rounded-2xl border border-stone-200/80 bg-white/90 backdrop-blur-sm p-6 sm:p-10 shadow-sm">
        <h2 class="text-xl font-title font-bold text-stone-800">评论 <span class="text-stone-400 font-normal text-base ml-1">({{ $article->approvedComments->count() }})</span></h2>

        <div class="mt-8 space-y-5">
            @forelse($article->approvedComments->sortByDesc('created_at') as $comment)
        ...
        <form action="{{ route('article.comment', $article) }}" method="POST" class="mt-8 pt-6 border-t border-stone-200/80 space-y-4">
            @csrf
            ...
            <textarea name="content" rows="4" placeholder="写下你的想法..." required
                      class="w-full rounded-xl border-stone-200 bg-stone-50/50 text-sm px-4 py-3 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors resize-none">{{ old('content') }}</textarea>
