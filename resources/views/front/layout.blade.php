...
<head>
    ...
    {{-- ----- SEO meta ----- --}}
    @php
        $metaDescription = trim(strip_tags($__env->yieldContent('description', config('app.description', '个人博客，记录文字、日常与技术分享。'))));
        // og:image：页面如未指定就用站点默认图（不存在则不输出 image，避免 404）
        $defaultOg = public_path('og-default.png');
        $defaultOgUrl = file_exists($defaultOg) ? asset('og-default.png') : null;
        $metaImage = trim($__env->yieldContent('og_image', $defaultOgUrl ?: ''));
...
<body class="min-h-screen flex flex-col bg-page text-slate-800 antialiased">
    <header class="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-md shadow-sm">
        <div class="max-w-6xl mx-auto px-4 sm:px-5 h-[3.25rem] sm:h-14 flex items-center justify-between">
            <div class="flex items-center gap-5 sm:gap-8 min-w-0">
                <a href="{{ route('home') }}" class="flex items-center gap-2.5 font-semibold text-slate-700 truncate group" aria-label="首页">
    ...
    <main class="max-w-6xl mx-auto w-full px-4 sm:px-5 py-8 sm:py-10 flex-1 relative z-10">
        ...
        @if($errors->any())
            <div class="mb-6 rounded-xl border border-red-200/80 bg-red-50/90 text-red-900 px-4 py-3 text-sm shadow-sm backdrop-blur-sm" role="alert">
    ...
    <button type="button" id="back-to-top" aria-label="回到顶部"
        ...
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18"/></svg>
    </button>
