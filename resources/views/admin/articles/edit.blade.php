...
<div x-data="articleAutoSave()">
    ...
    <div x-show="hasBackup" x-cloak
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-sm font-medium text-amber-800">检测到未保存的修改</p>
                <p class="text-xs text-amber-600 mt-0.5">上次编辑于 <span x-text="backupTime"></span></p>
            </div>
        </div>
        <div class="flex gap-2">
            <button type="button" @click="restoreBackup()" class="px-3 py-1.5 bg-amber-500 text-white rounded-lg text-sm hover:bg-amber-600 transition">恢复修改</button>
    ...
    <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" @submit="clearBackup()">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <div class="lg:col-span-3 space-y-6">
                <div class="bg-white rounded-xl shadow-sm p-6">
                    <div class="mb-4">
                        <input type="text" name="title" x-model="title" value="{{ old('title', $article->title) }}" placeholder="文章标题"
                    ...
                    <div class="mb-4">
                        <textarea name="excerpt" rows="2" x-model="excerpt" placeholder="文章摘要（选填，用于列表展示）"
                                  class="w-full border-gray-200 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500">{{ old('excerpt', $article->excerpt) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="content" id="editor" rows="20"
                                  class="w-full border-gray-200 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500">{{ old('content', $article->content) }}</textarea>
                        @error('content')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm p-6">
                    ...
                    <div x-data="{ preview: '{{ $article->cover_image ? asset('storage/' . $article->cover_image) : '' }}' }">
                        ...
                        <div x-show="preview" class="relative">
                            <img :src="preview" class="w-full rounded-lg cursor-pointer" @click="document.getElementById('cover-input').click()">
                            <button type="button" @click="preview = ''; document.getElementById('cover-input').value = ''"
